...
void Menu(void) {
    ...
    std::cout << "1. Создать фигуру\n" << std::endl;
    std::cout << "2. Вывести информацию о всех фигурах\n" << std::endl;
    std::cout << "3. Удалить фигуру\n" << std::endl;
    std::cout << "4. Удалить все фигуры\n" << std::endl;
    std::cout << "0. Выход\n" << std::endl;
}
...
void Create_Figure(Array<Figure>& figures){
    ...
    if (type == 1){
        ...
        std::cin >> *rhombus;
        figures.Add(rhombus.release());
    } else if (type == 3){
        auto trapezoid = std::make_unique<Trapezoid>();
        std::cin >> *trapezoid;
        figures.Add(trapezoid.release());
    } else {
